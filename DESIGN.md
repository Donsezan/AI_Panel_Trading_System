...
| Portfolio cannot be valued (no fresh mark for a held position; a balance in a currency nothing can price; a depegged stablecoin) | `risk.aggregate` while building the valuation, on every cycle gate and every supervisor sweep | aggregate **frozen**: no new order may be sent, the cycle records `BLOCKED`, and a `RISK_EVENT` + alert fire once per transition. The kill switch is **not** tripped, no baseline moves and no state is written — this is ignorance, not a breach, and it clears itself when marks return. A reduce-only operator exit is unaffected (ADR 0027) |
| Drawdown breach | Tier-2 watchdog | kill switch (§6.6), measured on mark-to-market equity (ADR 0027) |
